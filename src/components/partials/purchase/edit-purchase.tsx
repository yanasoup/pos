...
import { CalendarIcon, SquarePenIcon, Trash2Icon } from 'lucide-react';
...
import { CreatePurchaseParams } from '@/hooks/purchase/useCreateMasterPurchase';
import { UseGetPurchasesParams } from '@/hooks/purchase/useGetPurchase';
import { useGetPurchase } from '@/hooks/purchase/useGetPurchaseDetail';
import { useUpdatePurchase } from '@/hooks/purchase/useUpdateMasterPurchase';
import {
  ceilToNearest100,
  formatNumber,
...
const EditPurchaseDialog: React.FC<DialogProps> = ({
  masterId,
  onSaveSuccess,
  queryKey,
  ...props
  ...
  }, [props.open, getPurchaseDataFn, form, masterId, uiuxState.apiToken]);

  React.useEffect(() => {
    form.reset();
    if (purchaseData) {
      const detailData: SaleCartItem[] = [];
      purchaseData.data.forEach((item: any) => {
        const detailItem: SaleCartItem = {
          pd_id: item.pd_id,
      ...
      const newPurchaseCart: PurchaseCart = {
        master: {
          purchase_id: purchaseData.data[0].id,
          purchase_no: purchaseData.data[0].purchase_no,
          purchase_date: format(
            purchaseData.data[0].purchase_date,
            'yyyy-MM-dd',
            { locale: id }
          ),
          supplier_id: purchaseData.data[0].supplier_id,
          supplier: purchaseData.data[0].supplier,
          notes: purchaseData.data[0].notes || '',
          isAutoPriceCheck: true,
          rate_harga_jual: 10,
        },
        detail: detailData,
      ...
      form.reset({
        purchase_no: purchaseData.data[0].purchase_no,
        purchase_date: purchaseData.data[0].purchase_date,
        supplier_id: purchaseData.data[0].supplier_id,
        supplier: purchaseData.data[0].supplier,
        notes: purchaseData.data[0].notes || '',
        isAutoPriceCheck: true,
      ...
      dispatch(setPurchaseCartEdit(newPurchaseCart));
    }
  }, [purchaseData, dispatch, form]);
  function addItemToCart(formData: FormData) {
    if (isEditMode) {
      updateCart();
    } else {
      const existingItem = uiuxState.purchaseCartEdit.detail.find(
      ...
      const newData: PurchaseCartItem & { master: { purchase_id: string } } = {
        master: {
        ...
        detail: {
          ...
          price:
            parseFloat((formData?.sale_price ?? '').replace(/\D/g, '')) ||
            parseFloat(formData.price.replace(/\D/g, '')),
        },
      };
      // console.log('newCartData', newData);
      dispatch(addToPurchaseCartEdit(newData));
      resetItemForm();
      setIsEditMode(false);
    }
  }
  const updateCart = () => {
    const updateData: PurchaseCartItem = {
      ...
      detail: {
        ...
        price: Number(form.getValues('sale_price')?.replace(/\D/g, '')),
      },
    };
    dispatch(updatePurchaseCartEdit(updateData));
    resetItemForm();
    setIsEditMode(false);
  };
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter') {
      e.preventDefault();
      if ((e.target as HTMLInputElement).name === 'purchase_no') {
        form.setFocus('product_code');
      } else if ((e.target as HTMLInputElement).name === 'product_code') {
        if (!isEditMode) {
          getProductByCode({
  ...
  const startEditHandler = (dataId: string) => {
    const cartItem = uiuxState.purchaseCartEdit.detail.find(
      (item) => item.product_id === dataId
    ) as SaleCartItem;

    setIsEditMode(true);
    form.setValue('pd_id', cartItem.pd_id || '');
  ...
  const confirmDeleteHandler = (productCode: string) => {
    dispatch(removeFromPurchaseCartEdit(productCode));
    setIsEditMode(false);
  };
  ...
  const handleConfirmSelectedSupplier = (item: string) => {
    ...
    setTimeout(() => {
      dispatch(
        updateMasterPurchaseCartEdit({
          purchase_id: uiuxState.purchaseCartEdit.master.purchase_id,
          purchase_no: form.getValues('purchase_no'),
          purchase_date: form.getValues('purchase_date') || '',
          supplier_id: form.getValues('supplier_id') || '',
          supplier: form.getValues('supplier'),
  ...
  type EditMasterPurchaseParam = CreatePurchaseParams & {
    data: { master: { purchase_id: string } };
  };
  const commitTransaction = async () => {
    ...
    const data: EditMasterPurchaseParam = {
      data: {
        master: {
          purchase_id: masterId,
        ...
        items: uiuxState.purchaseCartEdit.detail,
      },
      requestToken: uiuxState.apiToken!,
    };
    updateFn(data);
    // alert('data submited');
    // console.log('updateFn', data);
  };
  ...
  React.useEffect(() => {
    if (isUpdateSuccess) {
      ...
      form.reset();
      setIsEditMode(false);
      dispatch(resetPurchaseCartEdit());
  ...
  }, [isUpdateSuccess, form, uiuxState?.apiToken, dispatch, onSaveSuccess]);
        ...
        <DialogHeader>
            ...
            <Form {...form}>
              <form onSubmit={form.handleSubmit(addItemToCart)}>
                ...
                <div className='overflow-hidden rounded-tl-lg rounded-tr-lg border md:flex-row'>
                  ...
                  <div className='flex flex-col flex-wrap gap-0 border-t px-2 pb-4 md:flex-row md:gap-4'>
                    <FormField
                      control={form.control}
                      name='product_code'
                      render={({ field }) => (
          ...
          <DialogBody className='flex overflow-scroll rounded-none border-none'> */}
            <div className='h-[200px] min-h-[200px] overflow-y-scroll'>
              {uiuxState.purchaseCartEdit.detail && (
                <GridPurchaseDetail
                  purchaseData={uiuxState.purchaseCartEdit.detail}
